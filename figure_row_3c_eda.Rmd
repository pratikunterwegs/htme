...
```{r}
# get data
data <- list.files(path = "data", pattern = "tx",
                   full.names = T)
...
data = lapply(data, function(file){
  ...
  df[, time := as.POSIXct(time, tz = "Amsterdam", 
                          origin = "1970-01-01")]
  df[, day := as.numeric(
    round(
...
ggsave(
  figure_time_heatmap,
  filename = "figures/fig_row_3_eda_time.png",
  height = 2.75, width = 5
)
```
